...
def speak(text: str, lang: str = "en") -> bytes:
    """
    Converts text to speech and returns MP3 audio bytes (so Streamlit can
...
def build_confirmation_text(cart: list, menu_lookup: dict, lang: str = "en") -> str:
    """
    ...
    for entry in cart:
        ...
        total += price
        name = item["name_hi"] if lang == "hi" else item["name"]
